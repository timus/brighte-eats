...
const toServices = (rows: LeadServiceInstance[] = []) => rows.map((r) => r.getDataValue("service"))
...
export const resolvers = {
  Query: {
    ...
    lead: async (_parent: unknown, args: { id: string }) => {
      ...
      const row = (await leadRepository.getLeadById(leadId)) as LeadWithCustomerAndServices | null
  ...
  Mutation: {
    register: async (_parent: unknown, args: { input: RegisterInput }) => {
      ...
      try {
        ...
        log.info({ leadId: row.getDataValue("id") }, "graphql:register:success")
